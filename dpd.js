...
dpd.changed = dpd("/changed");
dpd.changed.on = function(ev, fn) {
  return dpd.on("changed" + ":" + ev, fn);
}
dpd.changed.once = function(ev, fn) {
  return dpd.once("changed" + ":" + ev, fn);
}
dpd.changed.off = function(ev, fn) {
  return dpd.off("changed" + ":" + ev, fn);
}
dpd.changed.rename = function (path, body, fn) {
  dpd.changed.exec("rename", path, body, fn);
}

dpd.delete = dpd("/delete");
dpd.delete.on = function(ev, fn) {
  return dpd.on("delete" + ":" + ev, fn);
}
dpd.delete.once = function(ev, fn) {
  return dpd.once("delete" + ":" + ev, fn);
}
dpd.delete.off = function(ev, fn) {
  return dpd.off("delete" + ":" + ev, fn);
}
dpd.delete.rename = function (path, body, fn) {
  dpd.delete.exec("rename", path, body, fn);
}

dpd.empty = dpd("/empty");
dpd.empty.on = function(ev, fn) {
  return dpd.on("empty" + ":" + ev, fn);
}
dpd.empty.once = function(ev, fn) {
  return dpd.once("empty" + ":" + ev, fn);
}
dpd.empty.off = function(ev, fn) {
  return dpd.off("empty" + ":" + ev, fn);
}
dpd.empty.rename = function (path, body, fn) {
  dpd.empty.exec("rename", path, body, fn);
}

dpd.emptyusers = dpd("/empty-users");
dpd.emptyusers.login = function(path, body, fn) {
  return dpd.emptyusers.exec("login", path, body, fn);
}
dpd.emptyusers.logout = function(path, body, fn) {
  return dpd.emptyusers.exec("logout", path, body, fn);
}
dpd.emptyusers.me = function(path, query, fn) {
  return dpd.emptyusers.get("me", path, query, fn);
}
dpd.emptyusers.on = function(ev, fn) {
  return dpd.on("empty-users" + ":" + ev, fn);
}
dpd.emptyusers.once = function(ev, fn) {
  return dpd.once("empty-users" + ":" + ev, fn);
}
dpd.emptyusers.off = function(ev, fn) {
  return dpd.off("empty-users" + ":" + ev, fn);
}

dpd.full = dpd("/full");
dpd.full.on = function(ev, fn) {
  return dpd.on("full" + ":" + ev, fn);
}
dpd.full.once = function(ev, fn) {
  return dpd.once("full" + ":" + ev, fn);
}
dpd.full.off = function(ev, fn) {
  return dpd.off("full" + ":" + ev, fn);
}
dpd.full.rename = function (path, body, fn) {
  dpd.full.exec("rename", path, body, fn);
}


dpd.internalclientdetail = dpd("/internal-client-detail");
dpd.internalclientdetail.on = function(ev, fn) {
  return dpd.on("internal-client-detail" + ":" + ev, fn);
}
dpd.internalclientdetail.once = function(ev, fn) {
  return dpd.once("internal-client-detail" + ":" + ev, fn);
}
dpd.internalclientdetail.off = function(ev, fn) {
  return dpd.off("internal-client-detail" + ":" + ev, fn);
}
dpd.internalclientdetail.rename = function (path, body, fn) {
  dpd.internalclientdetail.exec("rename", path, body, fn);
}

dpd.internalclientmaster = dpd("/internal-client-master");
dpd.internalclientmaster.on = function(ev, fn) {
  return dpd.on("internal-client-master" + ":" + ev, fn);
}
dpd.internalclientmaster.once = function(ev, fn) {
  return dpd.once("internal-client-master" + ":" + ev, fn);
}
dpd.internalclientmaster.off = function(ev, fn) {
  return dpd.off("internal-client-master" + ":" + ev, fn);
}
dpd.internalclientmaster.rename = function (path, body, fn) {
  dpd.internalclientmaster.exec("rename", path, body, fn);
}

dpd.mynewcoll = dpd("/mynewcoll");
dpd.mynewcoll.on = function(ev, fn) {
  return dpd.on("mynewcoll" + ":" + ev, fn);
}
dpd.mynewcoll.once = function(ev, fn) {
  return dpd.once("mynewcoll" + ":" + ev, fn);
}
dpd.mynewcoll.off = function(ev, fn) {
  return dpd.off("mynewcoll" + ":" + ev, fn);
}
dpd.mynewcoll.rename = function (path, body, fn) {
  dpd.mynewcoll.exec("rename", path, body, fn);
}

dpd.previous = dpd("/previous");
dpd.previous.on = function(ev, fn) {
  return dpd.on("previous" + ":" + ev, fn);
}
dpd.previous.once = function(ev, fn) {
  return dpd.once("previous" + ":" + ev, fn);
}
dpd.previous.off = function(ev, fn) {
  return dpd.off("previous" + ":" + ev, fn);
}
dpd.previous.rename = function (path, body, fn) {
  dpd.previous.exec("rename", path, body, fn);
}


dpd.recursive = dpd("/recursive");
dpd.recursive.on = function(ev, fn) {
  return dpd.on("recursive" + ":" + ev, fn);
}
dpd.recursive.once = function(ev, fn) {
  return dpd.once("recursive" + ":" + ev, fn);
}
dpd.recursive.off = function(ev, fn) {
  return dpd.off("recursive" + ":" + ev, fn);
}
dpd.recursive.rename = function (path, body, fn) {
  dpd.recursive.exec("rename", path, body, fn);
}

dpd.todos = dpd("/todos");
dpd.todos.on = function(ev, fn) {
  return dpd.on("todos" + ":" + ev, fn);
}
dpd.todos.once = function(ev, fn) {
  return dpd.once("todos" + ":" + ev, fn);
}
dpd.todos.off = function(ev, fn) {
  return dpd.off("todos" + ":" + ev, fn);
}
dpd.todos.rename = function (path, body, fn) {
  dpd.todos.exec("rename", path, body, fn);
}
